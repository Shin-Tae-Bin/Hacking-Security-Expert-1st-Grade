Hacking-Security-Expert-1st-Grade
해킹보안전문가 1급 자격증 취득을 위한 이론·실습 학습 기록
Study, TIL, and lab codes for Hacking Security Expert Level 1 certification.

📚 소개 (About)
본 저장소는 해킹보안전문가 1급 자격증 취득을 위한 체계적인 학습 과정을 기록하는 공간입니다.

🎯 저장소 목적
해킹보안전문가 1급 강의 전 과정의 개념정리, 실습코드, 참고자료를 체계적으로 정리

학습한 내용을 복습하며 개인적인 이해와 해석을 추가한 맞춤형 학습노트 구축

실습 결과물과 프로젝트를 통한 실무 역량 증명

지속적인 포트폴리오 개발 및 취업 준비 자료 활용

📖 학습 방식
이론 + 실습 병행: 개념 학습 후 즉시 실습으로 체화

문서화 중심: 모든 학습 내용을 마크다운으로 구조화하여 정리

점진적 발전: 기초부터 고급까지 단계별 학습 기록

복습 최적화: 언제든 다시 찾아볼 수 있는 개인 맞춤형 참고서 구축

📑 폴더 구조 (Repository Structure)
text
📁 Hacking-Security-Expert-1st-Grade/
├── 📁 01_Fundamentals/                     # 기초 개념 및 이론 정리
│   ├── 📄 01_해킹개념.md                   # 해킹의 정의, 분류, 기본 개념
│   ├── 📄 02_보안취약점_개념_및_종류.md     # 보안취약점 분류 및 특징
│   └── 📁 images/                          # 개념 설명용 이미지 자료
│
├── 📁 02_Server_Security/                  # 서버 보안 (UNIX/Windows)
│   ├── 📁 UNIX/                           # UNIX 시스템 보안
│   │   ├── 📄 03_UNIX_보안취약점_01.md
│   │   ├── 📄 04_UNIX_보안취약점_02.md
│   │   ├── 📄 05_UNIX_보안취약점_03.md
│   │   └── 📁 scripts/                    # UNIX 보안 스크립트
│   ├── 📁 Windows/                        # Windows 시스템 보안
│   │   ├── 📄 06_Windows_보안취약점_01.md
│   │   ├── 📄 07_Windows_보안취약점_02.md
│   │   ├── 📄 08_Windows_보안취약점_03.md
│   │   ├── 📄 09_Windows_보안취약점_04.md
│   │   ├── 📄 10_Windows_보안취약점_05.md
│   │   └── 📁 tools/                      # Windows 보안 도구
│   └── 📄 README.md                       # 서버 보안 섹션 가이드
│
├── 📁 03_Network_Security/                 # 네트워크 및 장비 보안
│   ├── 📄 11_네트워크장비_보안취약점_01.md
│   ├── 📄 12_네트워크장비_보안취약점_02.md
│   ├── 📄 13_네트워크장비_보안취약점_03.md
│   ├── 📄 14_네트워크장비_보안취약점_04.md
│   ├── 📄 15_네트워크장비_보안취약점_05.md
│   ├── 📁 network_configs/                # 네트워크 설정 파일 모음
│   └── 📄 README.md                       # 네트워크 보안 섹션 가이드
│
├── 📁 04_Application_Security/             # 애플리케이션 보안
│   ├── 📄 16_애플리케이션_보안취약점_01.md
│   ├── 📄 17_애플리케이션_보안취약점_02.md
│   ├── 📄 18_애플리케이션_보안취약점_03.md
│   ├── 📄 19_애플리케이션_보안취약점_04.md
│   ├── 📄 20_애플리케이션_보안취약점_05.md
│   ├── 📁 web_labs/                       # 웹 보안 실습 파일
│   └── 📄 README.md                       # 애플리케이션 보안 섹션 가이드
│
├── 📁 05_Information_Security_Management/  # 정보보안 관리
│   ├── 📄 21_정보보안_개요.md
│   ├── 📄 22_정보보안_역사_및_필요성_목적.md
│   ├── 📄 23_개인정보보호의_이해_01.md
│   ├── 📄 24_개인정보보호의_이해_02.md
│   ├── 📄 25_개인정보보호의_이해_03.md
│   ├── 📁 legal_docs/                     # 관련 법령 및 규정 자료
│   └── 📄 README.md                       # 정보보안 관리 섹션 가이드
│
├── 📁 Lab_Projects/                        # 실습 프로젝트
│   ├── 📁 penetration_testing/            # 모의해킹 프로젝트
│   ├── 📁 vulnerability_assessment/       # 취약점 진단 프로젝트
│   ├── 📁 incident_response/              # 사고 대응 시나리오
│   └── 📄 README.md                       # 실습 프로젝트 가이드
│
├── 📁 Practice_Tests/                      # 시험 대비
│   ├── 📁 mock_exams/                     # 모의고사
│   ├── 📁 question_bank/                  # 문제은행
│   ├── 📁 my_answers/                     # 개인 답안 및 해설
│   └── 📄 README.md                       # 시험 대비 가이드
│
├── 📁 Resources/                           # 학습 자료
│   ├── 📄 study_materials.md              # 교재 및 참고서 정리
│   ├── 📄 useful_tools.md                 # 보안 도구 목록 및 사용법
│   ├── 📄 online_resources.md             # 온라인 학습 자료 링크
│   ├── 📄 exam_info.md                    # 시험 정보 및 일정
│   └── 📄 README.md                       # 자료 활용 가이드
│
├── 📁 TIL/                                 # Today I Learned
│   ├── 📄 2025-01-Week1.md               # 주차별 학습 기록
│   ├── 📄 2025-01-Week2.md
│   ├── 📄 study_reflection.md             # 학습 회고 및 느낀점
│   └── 📄 README.md                       # TIL 작성 규칙 및 가이드
│
└── 📄 README.md                            # 메인 프로젝트 가이드 (현재 파일)
🗂️ 강의 커리큘럼 전체 목록
📌 1-2강: 기초 개념 (Fundamentals)
1강: 해킹 개념

2강: 보안취약점의 개념 및 종류

🖥️ 3-10강: 서버 보안 (Server Security)
UNIX 시스템 보안 (3-5강)

3강: 보안취약점(서버보안 UNIX) 01

4강: 보안취약점(서버보안 UNIX) 02

5강: 보안취약점(서버보안 UNIX) 03

Windows 시스템 보안 (6-10강)

6강: 보안취약점(서버보안 Windows) 01

7강: 보안취약점(서버보안 Windows) 02

8강: 보안취약점(서버보안 Windows) 03

9강: 보안취약점(서버보안 Windows) 04

10강: 보안취약점(서버보안 Windows) 05

🌐 11-15강: 네트워크 보안 (Network Security)
11강: 보안취약점(보안, 네트워크장비) 01

12강: 보안취약점(보안, 네트워크장비) 02

13강: 보안취약점(보안, 네트워크장비) 03

14강: 보안취약점(보안, 네트워크장비) 04

15강: 보안취약점(보안, 네트워크장비) 05

💻 16-20강: 애플리케이션 보안 (Application Security)
16강: 보안취약점(애플리케이션) 01

17강: 보안취약점(애플리케이션) 02

18강: 보안취약점(애플리케이션) 03

19강: 보안취약점(애플리케이션) 04

20강: 보안취약점(애플리케이션) 05

📋 21-25강: 정보보안 관리 (Information Security Management)
21강: 정보보안 개요

22강: 정보보안의 역사 및 필요성, 목적

23강: 개인정보보호의 이해 01

24강: 개인정보보호의 이해 02

25강: 개인정보보호의 이해 03

📝 상세 기록 방식 및 사용법
✍️ 마크다운 파일 작성 규칙
text
# [강의번호] 강의명

## 📖 학습 목표
- 이번 강의에서 달성해야 할 목표 명시

## 🔑 핵심 개념
- 주요 용어 정의
- 중요 개념 설명
- 관련 이론 정리

## 💡 상세 내용
### 세부 주제 1
- 구체적인 설명
- 예시 및 사례

### 세부 주제 2
- 실습 과정
- 코드 예제 (해당시)

## ⚠️ 주의사항 및 취약점
- 보안상 주의해야 할 점
- 일반적인 실수 사례

## 🔧 실습 및 예제
코드 예제 (해당시)
실습 명령어나 스크립트

text

## 📚 참고자료
- 관련 링크
- 추가 학습 자료

## 🤔 개인 노트 및 회고
- 어려웠던 부분
- 새롭게 알게 된 점
- 실무 적용 방안
📁 TIL (Today I Learned) 작성법
text
# 2025-01-XX 학습 기록

## 📚 오늘 학습한 강의
- X강: 강의명

## 🎯 주요 학습 내용
- 핵심 포인트 1
- 핵심 포인트 2
- 핵심 포인트 3

## 💭 새롭게 알게 된 것
- 이전에 몰랐던 개념이나 방법

## 🤔 어려웠던 부분
- 이해하기 힘들었던 개념
- 추가 학습이 필요한 부분

## 🔄 복습이 필요한 내용
- 다시 봐야 할 부분
- 실습으로 보완할 내용

## 📝 내일/다음 학습 계획
- 다음에 학습할 내용
- 보완하고 싶은 부분
🔧 실습 프로젝트 기록법
text
# 프로젝트명

## 🎯 목표
프로젝트의 목적과 달성하고자 하는 결과

## 🛠️ 사용 도구 및 환경
- 운영체제
- 사용 프로그램
- 필요 라이브러리

## 📋 실습 과정
### 1단계: 환경 설정
### 2단계: 실행
### 3단계: 결과 분석

## 📊 결과 및 스크린샷
실습 결과 이미지나 로그

## 🔍 분석 및 해석
결과에 대한 개인적 분석

## 📝 학습 포인트
이 실습을 통해 배운 점
💡 목적 및 활용 안내
🎯 단기 목표 (학습 중)
체계적 복습: 강의 내용을 단계별로 정리하여 효율적인 복습 지원

개념 정립: 각 주제별 핵심 개념을 명확히 이해하고 문서화

실습 역량 강화: 이론을 실습으로 연결하여 실무 역량 개발

약점 보완: 어려웠던 부분을 중점적으로 재학습하고 보완

🚀 장기 목표 (학습 후)
포트폴리오 구축: 체계적인 학습 과정을 보여주는 개인 포트폴리오

지식 베이스: 언제든 참조할 수 있는 개인 맞춤형 보안 지식 베이스

취업 준비: 면접이나 실무에서 활용할 수 있는 구체적인 경험과 지식

지속적 학습: 새로운 보안 기술 학습 시 기반이 되는 체계적 틀

📖 활용 방법
일일 학습: 강의 수강 후 즉시 해당 폴더에 내용 정리

주간 복습: TIL을 통해 주간 학습 내용 회고 및 정리

프로젝트 실습: Lab_Projects 폴더에서 실무형 프로젝트 진행

시험 대비: Practice_Tests 폴더로 체계적인 시험 준비

지속적 업데이트: 새로운 내용 학습 시 관련 섹션에 추가 정리

🔄 지속적 개선 및 업데이트
이 저장소는 살아있는 학습 기록입니다. 새로운 내용을 배우거나 기존 내용을 더 깊이 이해하게 되면 지속적으로 업데이트됩니다.

📧 연락처: [개인 연락처 또는 GitHub 이슈를 통한 피드백 환영]

💪 "기록하는 자가 기억하고, 기억하는 자가 성장한다"
자세한 기록 기준, 커밋 메시지 규칙, 실습 예시는 각 폴더의 README.md를 참고하세요.
